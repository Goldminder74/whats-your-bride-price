...
test("repository contains no inaccurate permanent-result promise and exactly one Prompt 22 migration",async()=>{
  ...
  const migrations=await readdir(new URL("../../drizzle/",import.meta.url));assert.deepEqual(migrations.filter((name)=>/^0007_.*\.sql$/.test(name)),["0007_ancient_yellow_claw.sql"]);assert.deepEqual(migrations.filter((name)=>/^0008_.*\.sql$/.test(name)),["0008_simple_nocturne.sql"]);assert.deepEqual(migrations.filter((name)=>/^0009_.*\.sql$/.test(name)),["0009_clammy_shooting_star.sql"]);assert.equal(migrations.some((name)=>/^0010_/.test(name)),false);
});
