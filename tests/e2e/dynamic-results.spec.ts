...
test("three regional public results render distinctly without JavaScript", async ({ browser }) => {
  ...
  for (const [slug, label, score] of [[slugs.west, "West Africa", "12"], [slugs.east, "East Africa", "8"], [slugs.north, "North Africa", "4"]]) {
    await page.goto(`/result/${slug}`); await expect(page.locator("[data-public-result]")).toBeVisible(); await expect(page.getByText(label, { exact: false }).first()).toBeVisible(); await expect(page.locator(".public-result-score strong")).toHaveText(score); await expect(page.getByText("A playful culture score, never a measure of human worth.")).toBeVisible();
  }
  await context.close();
});
...
test("Share Centre requires explicit publication and can return the result to private", async ({ page }) => {
  ...
  await centre.getByRole("button", { name: "Make result public" }).click(); await expect(centre.locator("[data-result-publication]")).toHaveAttribute("data-publication-visibility", "public"); await expect(centre.locator(".share-centre-link-mode")).toContainText("Permanent public result");
  await centre.getByRole("button", { name: "Unpublish result" }).click(); await expect(centre.locator("[data-result-publication]")).toHaveAttribute("data-publication-visibility", "private"); await expect(centre.locator(".share-centre-link-mode")).not.toContainText("Permanent public result");
});
