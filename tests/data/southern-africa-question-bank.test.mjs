...
const importFile = new URL("../../data/question-bank/southern-africa/southern-africa-draft-v1.json", import.meta.url);
const reviewFile = new URL("../../data/question-bank/southern-africa/southern-africa-cultural-review.csv", import.meta.url);
const sourceFile = new URL("../../data/question-bank/southern-africa/southern-africa-source-register.csv", import.meta.url);
const priorFiles = ["west-africa/west-africa-draft-v1.json", "east-africa/east-africa-draft-v1.json", "north-africa/north-africa-draft-v1.json", "central-africa/central-africa-draft-v1.json"].map((path) => new URL(`../../data/question-bank/${path}`, import.meta.url));

async function loadDocument() { return parseQuestionBankJson(await readFile(importFile, "utf8")); }
function sha256(value) { return createHash("sha256").update(value).digest("hex"); }

test("Southern Africa import contains 68 schema-valid draft candidates with unique answers and versions", async () => {
  const document = await loadDocument();
  assert.equal(document.questions.length, 68);
  assert.equal(validateQuestionBankDocument(document).questions.length, 68);
  const versions = new Set();
  for (const question of document.questions) {
    assert.equal(question.region, "south");
...
test("difficulty, topic, country and specialist-review coverage matches the research brief", async () => {
  ...
  assert.deepEqual(difficulty, { introductory: 28, intermediate: 27, advanced: 13 });
  const categories = new Set(document.questions.map((question) => question.category));
  for (const required of ["ART", "ART & HISTORY", "FOOD", "GEOGRAPHY", "HISTORY", "LANGUAGE", "MODERN HISTORY", "MUSIC", "ORAL HISTORY", "PEOPLES & LANGUAGE", "PHILOSOPHY", "TEXTILES", "TRADITION"]) assert.ok(categories.has(required));
  const countries = new Set(document.questions.flatMap((question) => question.countryScope.split("; ")));
  for (const country of ["Botswana", "Eswatini", "Lesotho", "Namibia", "South Africa", "Zambia", "Zimbabwe"]) assert.ok(countries.has(country));
  assert.ok(document.questions.every((question) => question.communityScope));
  assert.equal(document.questions.filter((question) => question.sensitivityNotes).length, 68);
...
test("every candidate has a question-specific UNESCO source and source-register entry", async () => {
  const document = await loadDocument();
  const sourceUrls = new Set();
  for (const question of document.questions) {
    assert.equal(question.sources.length, 1);
    const source = question.sources[0];
    assert.match(source.urlOrReference, /^https:\/\/(?:ich|whc)\.unesco\.org\//);
  ...
  assert.equal(sourceUrls.size, 17);
  const sourceLines = (await readFile(sourceFile, "utf8")).trimEnd().split(/\r?\n/);
  assert.equal(sourceLines.length, 18);
...
test("draft candidates cannot enter public selection", async () => {
  const document = await loadDocument();
  const candidates = document.questions.map((question, index) => ({ ...question, internalId: `draft_${index}`, editionId: "edition_south_v1", sourceReviewStatus: "approved" }));
  await assert.rejects(selectQuestionSet({ candidates, region: "south", now: Date.UTC(2026, 8, 6) }), /insufficient_published_bank/);
});

test("Southern candidates have no exact or configured near duplicates against all earlier catalogues", async () => {
  const document = await loadDocument();
  ...
  assert.equal(report.questionCount, 68);
...
test("JSON and CSV formats round-trip while review decisions stay pending", async () => {
  ...
  assert.equal(parseQuestionBankCsv(csv).questions.length, 68);
  assert.equal(parseQuestionBankJson(JSON.stringify(document)).questions.length, 68);
  ...
  assert.equal((review.match(/,pending,,,/g) || []).length, 68);
...
test("prior packs, protected files and migrations retain approved hashes", async () => {
  const protectedFiles = new Map([
    ...
    [priorFiles[3], "4692ddd5c3f2b098da52c7ea4845776df021b1715462fc31225ba6dca8801c13"],
  ]);
