...
const importFile = new URL("../../data/question-bank/north-africa/north-africa-draft-v1.json", import.meta.url);
const reviewFile = new URL("../../data/question-bank/north-africa/north-africa-cultural-review.csv", import.meta.url);
const sourceFile = new URL("../../data/question-bank/north-africa/north-africa-source-register.csv", import.meta.url);
const westImportFile = new URL("../../data/question-bank/west-africa/west-africa-draft-v1.json", import.meta.url);
const eastImportFile = new URL("../../data/question-bank/east-africa/east-africa-draft-v1.json", import.meta.url);
...
test("North Africa import contains 66 schema-valid draft candidates with unique answers and versions", async () => {
  const document = await loadDocument();
  assert.equal(document.questions.length, 66);
  assert.equal(validateQuestionBankDocument(document).questions.length, 66);
  const versions = new Set();
  for (const question of document.questions) {
    assert.equal(question.region, "north");
...
test("difficulty, category, scope and specialist-review coverage matches the generated reports", async () => {
  ...
  assert.deepEqual(difficulty, { introductory: 27, intermediate: 26, advanced: 13 });
  const categories = new Set(document.questions.map((question) => question.category));
  for (const required of ["FOOD", "TRADITION", "MUSIC", "TEXTILES", "ART", "HISTORY", "GEOGRAPHY", "LANGUAGE", "ORAL HISTORY"]) assert.ok(categories.has(required));
  ...
  assert.equal(specialist.length, 63);
...
test("draft candidates cannot enter public selection", async () => {
  ...
  const candidates = document.questions.map((question, index) => ({
    ...
    editionId: "edition_north_v1",
    sourceReviewStatus: "approved",
  }));
  await assert.rejects(selectQuestionSet({ candidates, region: "north", now: Date.UTC(2026, 8, 6) }), /insufficient_published_bank/);
});

test("new candidates have no exact or configured near duplicates against all earlier catalogues", async () => {
  const document = await loadDocument();
  const legacy = await buildLegacyQuestionBankDocument();
  const west = parseQuestionBankJson(await readFile(westImportFile, "utf8"));
  const east = parseQuestionBankJson(await readFile(eastImportFile, "utf8"));
  const comparisonCatalogue = validateQuestionBankDocument({ schemaVersion: "question-bank-v1", questions: [...legacy.questions, ...west.questions, ...east.questions] });
  const report = previewQuestionBankImport(document, comparisonCatalogue);
  assert.equal(report.questionCount, 66);
...
test("JSON and CSV import formats round-trip and CSV neutralisation remains active", async () => {
  ...
  assert.equal(parseQuestionBankCsv(csv).questions.length, 66);
  assert.equal(parseQuestionBankJson(JSON.stringify(document)).questions.length, 66);
