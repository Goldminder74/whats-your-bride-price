...
export const commerceOrders = sqliteTable("commerce_orders", {
  ...
  resultId: text("result_id").references(() => results.id, { onDelete: "restrict" }),
  cowrieWalletId: text("cowrie_wallet_id").references(() => cowrieWallets.id, { onDelete: "restrict" }),
  stripePaymentIntentHash: text("stripe_payment_intent_hash"),
  ...
  uniqueIndex("commerce_orders_payment_intent_hash_uq").on(table.stripePaymentIntentHash).where(sql`${table.stripePaymentIntentHash} is not null`),
  ...
  check("commerce_orders_product_price_ck", sql`${table.currency} = 'GBP' and typeof(${table.amountMinor}) = 'integer' and ((${table.productKey} in ('royal_reveal_v1','cowrie_5_v1') and ${table.amountMinor} = 199) or (${table.productKey} = 'cowrie_15_v1' and ${table.amountMinor} = 499) or (${table.productKey} = 'cowrie_40_v1' and ${table.amountMinor} = 999))`),
  check("commerce_orders_target_ck", sql`(${table.productKey} = 'royal_reveal_v1' and ${table.resultId} is not null and ${table.cowrieWalletId} is null) or (${table.productKey} in ('cowrie_5_v1','cowrie_15_v1','cowrie_40_v1') and ${table.resultId} is null and ${table.cowrieWalletId} is not null)`),
  check("commerce_orders_payment_intent_hash_ck", sql`${table.stripePaymentIntentHash} is null or (length(${table.stripePaymentIntentHash}) = 64 and ${table.stripePaymentIntentHash} not glob '*[^0-9a-f]*')`),
...
export const cowriePurchaseAllocations = sqliteTable("cowrie_purchase_allocations", {
  ...
  check("cowrie_allocations_product_ck", sql`${table.productKey} = 'cowrie_pack_v1' or (${table.productKey} = 'cowrie_5_v1' and ${table.originalQuantity} = 5) or (${table.productKey} = 'cowrie_15_v1' and ${table.originalQuantity} = 15) or (${table.productKey} = 'cowrie_40_v1' and ${table.originalQuantity} = 40)`),
...
export const stripeWebhookEvents = sqliteTable("stripe_webhook_events", {
  ...
  paymentIntentHash: text("payment_intent_hash"),
  amountMinor: integer("amount_minor"),
  amountRefundedMinor: integer("amount_refunded_minor"),
  ...
  index("stripe_webhook_events_payment_intent_received_idx").on(table.paymentIntentHash, table.receivedAt).where(sql`${table.paymentIntentHash} is not null`),
  check("stripe_webhook_events_payment_intent_hash_ck", sql`${table.paymentIntentHash} is null or (length(${table.paymentIntentHash}) = 64 and ${table.paymentIntentHash} not glob '*[^0-9a-f]*')`),
  check("stripe_webhook_events_amount_ck", sql`${table.amountMinor} is null or (typeof(${table.amountMinor}) = 'integer' and ${table.amountMinor} between 0 and 9007199254740991)`),
  check("stripe_webhook_events_refunded_amount_ck", sql`${table.amountRefundedMinor} is null or (typeof(${table.amountRefundedMinor}) = 'integer' and ${table.amountRefundedMinor} between 0 and 9007199254740991 and (${table.amountMinor} is null or ${table.amountRefundedMinor} <= ${table.amountMinor}))`),
