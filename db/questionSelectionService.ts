...
export function validatePublicSelectionRequest(value: unknown): Readonly<{ region: RegionKey; anonymousSubjectHash: string; idempotencyKey: string }> {
  if (!value || typeof value !== "object" || Array.isArray(value)) return fail("selection_request_invalid");
  const candidate = value as Record<string, unknown>;
  if (Object.keys(candidate).sort().join(",") !== "anonymousSubjectHash,idempotencyKey,region") return fail("selection_request_field_not_allowed");
  ...
  if (typeof candidate.anonymousSubjectHash !== "string" || !HASH.test(candidate.anonymousSubjectHash)) return fail("selection_subject_invalid");
  ...
  return Object.freeze({ region: candidate.region as RegionKey, anonymousSubjectHash: candidate.anonymousSubjectHash, idempotencyKey: candidate.idempotencyKey });
}
...
export class QuestionSelectionService {
  ...
  async start(value: unknown): Promise<PublicQuestionSelection> {
    ...
    const [candidates, recent] = await Promise.all([
      ...
      this.repository.getRecentQuestionVersions(request.anonymousSubjectHash),
    ]);
    ...
    const created = await this.repository.createAttempt({
      id: `attempt_${hex(internalIdBytes)}`,
      editionId: selection.questions[0].editionId,
      anonymousSubjectHash: request.anonymousSubjectHash,
      idempotencyKeyHash: await sha256(`wybp-question-selection-v1\u0000${request.anonymousSubjectHash}\u0000${request.idempotencyKey}`),
      selection,
      startedAt: now,
