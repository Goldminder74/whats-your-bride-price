...
export default function CowrieWalletPanel({ refreshKey }: Readonly<{ refreshKey?: string }>) {
  ...
  return <aside className="cowrie-wallet-control" data-cowrie-wallet>
    <button type="button" className="cowrie-wallet-toggle" aria-expanded={open} onClick={() => setOpen((value) => !value)}><span aria-hidden="true">◉</span> Cowries <b>{wallet?.totalBalance ?? "—"}</b></button>
    {open && <section className="cowrie-wallet-panel" aria-labelledby="cowrie-wallet-title">
      <header><div><p>Closed-loop play access</p><h2 id="cowrie-wallet-title">Cowrie Wallet</h2></div><button type="button" aria-label="Close Cowrie Wallet" onClick={() => setOpen(false)}>×</button></header>
      {wallet ? <>
      ...
      <p className="cowrie-no-value">Cowries have no cash value. They cannot be transferred, resold or withdrawn. A future one-off purchase option requires separate approval.</p>
