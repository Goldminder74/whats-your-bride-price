...
The Cowrie Wallet adds no browser storage key. It reuses the existing 24-hour anonymous functional session for owner proof. A recovery credential exists in component memory only during the creation or rotation response and enters the clipboard or neutral `wallet-recovery.txt` download only after an explicit player action. The public wallet reference and raw recovery value never enter URLs, analytics, logs, cookies, localStorage, generated media or another user’s HTML.
