...
Prompt 19 adds browser privacy controls, a public-safe storage inventory and draft legal routes only. It creates no schema or migration: migrations remain exactly `0000` through `0006`, with no `0007`. `.openai/hosting.json` remains `d1: null` and `r2: null`; no hosted database, object store, migration, seed or retention process was accessed or activated. Production still requires every gate in `docs/legal-content-activation-checklist.md`, explicit binding and migration approval, and a separate deployment approval.
