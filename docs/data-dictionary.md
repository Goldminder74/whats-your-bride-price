...
## Cowrie economy tables

`cowrie_wallets` holds internal `id`, opaque `public_reference`, `anonymous_owner_hash`, controlled `state`, `free_quick_plays_consumed`, separate `purchased_balance` and `bonus_balance`, `recovery_credential_hash`, `recovery_credential_version`, internal access-idempotency marker, lifecycle and retention timestamps, optimistic `version`, and created or updated timestamps. Only active or frozen records participate in the unique owner relationship. Internal IDs and hashes never enter the public projection.

`cowrie_ledger` is append-only. Fields are internal `id`, `wallet_id`, controlled `bucket`, controlled `entry_type`, signed `delta`, controlled `idempotency_domain`, unique domain-scoped `idempotency_hash`, optional authoritative attempt, achievement, order and allocation relationships, controlled `reason_code`, optional exact bonus expiry, optional reversal relationship, and `created_at`. Triggers reject update and delete, inactive-wallet writes and negative cached balances; each insert reconciles exactly one bucket cache.

`cowrie_purchase_allocations` reserves one verified commerce order, one wallet, product `cowrie_pack_v1`, original and remaining quantity, controlled state, fulfilment idempotency hash, fulfilment, refund, dispute, deletion and retention timestamps, optimistic version, and created or updated timestamps. Workstream C creates no allocations.

The nullable quiz_attempts.cowrie_issued_at column is a positive integer UTC epoch millisecond within the JavaScript Date range, no earlier than started_at. It records server commitment of the safe paid projection for return, not browser receipt or completion. Initial inserts must leave it null; authority and immutability triggers guard its one transition. Historical, free and non-Cowrie attempts may remain null. It is never public, browser supplied, logged or used for analytics or retention.

## Disabled Cowrie commerce (0010)

Orders now have nullable result/wallet targets with exact mutually exclusive controlled-product checks and a nullable domain-separated Payment Intent hash. Webhook audit adds only nullable payment_intent_hash, amount_minor and amount_refunded_minor; historical values stay null. Allocation products now enforce exact 5/15/40 quantities, while existing legacy allocations are preserved. Negative refund_reversal ledger entries distinguish verified_refund and verified_dispute. Frozen-wallet exceptions admit only exact verified allocation settlement; dispute_freeze stays zero-only and blocked. See [Cowrie commerce](cowrie-commerce.md) for authority, transaction, migration and privacy details.
