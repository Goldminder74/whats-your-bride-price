...
## Prompt 19 privacy-controls integration

The stricter Prompt 16 opt-in model is preserved. `first_party_analytics` remains false by default; no identifier, storage or transmission occurs before Allow; Do not allow does not impair the quiz; and withdrawal immediately removes the raw analytics credential and stops further transmission. Privacy choices is site-wide and Clear my local data resets analytics without emitting an event or creating a new identifier. The notice version remains `analytics-notice-v1`, choices persist for 180 days, analytics sessions for at most 24 hours, and raw version-1 events for no more than 30 days. Marketing and advertising remain absent.

No UK statistical-purpose exception is relied upon. Any future change would require separate owner and professional review of purpose limitation, individual-level retention, aggregation, objection controls and lawful basis, plus a new notice version where purpose or inventory materially changes.
