...
An explicitly published result uses `/result/{48 lowercase hexadecimal characters}`. Facebook, WhatsApp, Copy link and Native share use that same URL. It expires exactly 90 days after authoritative quiz completion; publishing never restarts, extends or removes that expiry. Publication is never triggered by opening the Share Centre, page rendering, metadata, media preparation or crawler traffic. The player sees the public-link, expiry, unpublication and external-cache disclosure and must select Make result public. Keeping it private preserves the existing challenge or neutral regional fallback. Unpublishing immediately returns the Share Centre to its fallback and makes the origin page and generated media unavailable. External previews and downloaded or shared copies may persist outside application control.
